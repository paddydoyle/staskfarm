...
SCRATCHDIR="/localscratch/${SLRUM_JOB_ID}"

print_help ()
{
    echo "The general script's help msg"
    printf "Usage: $(basename $0) [-t|--threads <arg>] [--(no-)verbose] [-v|--version] [-h|--help] <comand> [path(s)]\n"
    printf "\t%s\n" "<comand>: the command to be carried out in the task farm"
...
######################################################
if [[ ${#_arg_leftovers[@]} -eq 0 && "x${SLRUM_NTAKS}" = "x" ]]; then
...
if [[ ${#_arg_leftovers[@]} -gt 0 ]]; then
    ...
    while [ $iter < $_our_args ]; do 
        ...
        for (( i = 0; i < $SLRUM_NTASKS || $iter < $_our_args; i++ )); do
